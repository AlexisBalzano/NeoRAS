...
namespace ras {
void NeoRAS::RegisterTagActions()
{
    ...
    tagInterface_->SetActionDropdown(tagMenuId_, dropdownDef);
}

void NeoRAS::OnTagAction(const PluginSDK::Tag::TagActionEvent *event)
{
...
void NeoRAS::OnTagDropdownAction(const PluginSDK::Tag::DropdownActionEvent *event)
{
    if (!initialized_ || !event )
    {
        return;
...
void NeoRAS::TagProcessing(const std::string &callsign, const std::string &actionId, const std::string &userInput)
{
